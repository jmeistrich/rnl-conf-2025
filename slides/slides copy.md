...
<!--
I spent years building the fastest list library to make apps faster, but a faster list is actually not the biggest performance problem in most apps.
-->
...
<!--
We use useCallback and useMemo and enable the compiler, and assume that's good enough.
-->
...
It never re-renders the outer list, or the array of containers. It signals individual containers to re-render themselves at a new position with a new item as they come onto screen.

The whole goal is to re-render as little and small as possible.
-->
...
Compiler and memoizing can't save you here. The value is actually changing, so React does a ton of work rendering and reconciling everything.
...
<!--
Context can be better in some cases, it lets you separate concerns better.
-->
...
<!--
I've seen this absolutely destroy performance. If any value in a context changes, it re-renders everything, even if they aren't used.
...
<!--
But then you end up with these massive context trees. And they still re-render too much anyway.
-->
...
<!--
But how much does this matter? Bow big of a deal is some extra renders? The React docs say to do this, so it must be fine.
...
<!--
I did a little benchmark in a music app I'm making, updating the timestamp two times a second. In the naive one it passes state down from the root. Or it owns the state in a medium size component in the middle of the tree. Or just a single text element leaf node updates itself.
...
This really is the biggest problem in most apps.
...
<!--
I've consulted with many companies who say the built-in state
-->
...
<!--
You might know me from LegendList.
...
<div class="render-tree wide-tree" style="--flash-duration: 2s">
  <div class="render-node waste">MusicScreen</div>
  <div class="render-children">
    <div>
      <div class="render-node waste">TrackList</div>
...
<div class="mt-10 text-3xl text-gray-300">Use a state library. Use Legend State.</div>
